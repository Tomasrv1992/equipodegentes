import type { MonitorReport, ClienteCheckResult } from "./monitor";

/**
 * Genera HTML + subject del email diario del monitor al admin.
 */
export function buildMonitorEmail(report: MonitorReport): {
  subject: string;
  html: string;
  text: string;
} {
...
function buildPlainText(report: MonitorReport): string {
  const lines: string[] = [
