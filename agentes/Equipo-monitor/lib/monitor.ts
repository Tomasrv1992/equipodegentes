/**
 * Equipo-monitor — agente diario que valida la ejecución del cron de
...
/**
 * Genera el reporte completo del monitor.
...
export async function runMonitor(): Promise<MonitorReport> {
  ...
  const { data: zombies, error: zErr } = await supa
    ...
    .update({
      ...
      error_message: `Zombie cerrado por monitor (running > ${RUNNING_TIMEOUT_MIN}min)`,
    })
    .eq("status", "running")
  ...
  if (zErr) {
    console.warn("[monitor] error cerrando zombies:", zErr.message);
  }
